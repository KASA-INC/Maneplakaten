...
try:
    ...
    MOON_TEX = None
NAVY = (22, 21, 37); CREAM = (255, 238, 222); LITNAVY = (36, 35, 55)
...
def moon_layer(W, H, cx, cy, r, p, flip=1):
    ...
    # volum pa mork side: lyskilde mot belyst kant
    lx = cx + flip*(-0.3*r if p < 0.5 else 0.3*r); ly = cy - 0.3*r
    yy, xx = np.mgrid[0:H, 0:W]
    val = np.clip(1 - np.sqrt((xx-lx)**2 + (yy-ly)**2)/(2*r), 0, 1)**1.6
    ...
    ImageDraw.Draw(ring).ellipse([cx-r, cy-r, cx+r, cy+r], outline=255, width=max(2, int(r*0.012)))
    ring = ring.filter(ImageFilter.GaussianBlur(r*0.05)).point(lambda v: int(v*0.28))
...
def build_image(date_str, place, fmt, dpi=300, flip=1):
    ...
    tracked(draw, weekday, load_font(Fs), cx, C+0.40*Fd-gap, Fs*0.30, CREAM)
    draw.text((cx, C+0.40*Fd), datestr, font=load_font(Fd), fill=CREAM, anchor="ms")
    tracked(draw, format_place(place), load_font(Fs), cx, C+0.40*Fd+gap, Fs*0.12, CREAM)
...
def render(date_str, place, fmt, out, dpi=300, flip=1):
    img, info = build_image(date_str, place, fmt, dpi, flip)
...
def render_bytes(date_str, place, fmt, dpi=300, flip=1):
